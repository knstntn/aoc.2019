...
const md1 = markGrid(grid, path1, 1);
const md2 = markGrid(grid, path2, 2);
...
function markGrid(grid, path, pv) {
    let md = Number.MAX_SAFE_INTEGER;
    let mSteps = Number.MAX_SAFE_INTEGER;
    let steps = 0;

    const segments = path.split(',').map(x => x.trim());
    const point = { ...start };
    for (const segment of segments) {
        const direction = segment.charAt(0);
        let value = Number(segment.substring(1));

        while (value > 0) {
            steps++;
            value -= 1;

            point.x += (direction === 'R' ? 1 : (direction === 'L' ? -1 : 0));
            point.y += (direction === 'U' ? 1 : (direction === 'D' ? -1 : 0));

            grid[point.x] = grid[point.x] || {};

            if (!grid[point.x][point.y]) {
                grid[point.x][point.y] = { value: pv, [pv]: steps };
            } else {
                if (grid[point.x][point.y].value === pv || grid[point.x][point.y].value === 3) {
                    grid[point.x][point.y][pv] = Math.min(steps, grid[point.x][point.y][pv]);
                } else {
                    // console.log('>>>', grid[point.x][point.y]);
                    grid[point.x][point.y].value += pv;
                    grid[point.x][point.y][pv] = steps;
                }
            }

            if (grid[point.x][point.y].value === 3) {
                // console.log('>>>', grid[point.x][point.y]);
                mSteps = Math.min(mSteps, grid[point.x][point.y][1] + grid[point.x][point.y][2]);
                md = Math.min(md, distance(point));
            }
        }
    }
    return { distance: md, steps: mSteps };
}
